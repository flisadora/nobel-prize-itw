define(['knockout'], function (ko) {
    var vm = function () {
        ...
        self.qtyPrizes = ko.observable();
        self.qtyMen = ko.observable();
        self.qtyWomen = ko.observable();
        self.qtyNationalities = ko.observable();
                ...
                ajaxHelper(baseUri+'/'+elem.LaureadoId, 'GET').done(function (data) {
                    ...
                    for(var k = 0; k < data.PremioNobel.length; k++){
                        if(data.PremioNobel[k].Ano > from && data.PremioNobel[k].Ano < until){
                            aux.push(data);
                            if(data.Sexo == "F") women +=1;
                            if(data.Sexo == "M") men +=1;
                            if(!(data.CidadeNascimento.Pais.Nome in dic)) dic[data.CidadeNascimento.Pais.Nome] = data.CidadeNascimento.Pais.PaisId;
                        }
                    }
                    if(i == sizeOriginal){
                        self.laureates(aux);
